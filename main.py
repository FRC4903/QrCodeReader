...
while True:
    ...
    if(len(value) > 0 and lastValue != value):
        ...
        if(Vl[0] == "ps"):
            ...
            if(os.path.exists(f"Teams\\PitScouting{Vl[0]}.csv")):
                with open(f"Teams\\PitScouting{Vl[0]}.csv", "a", encoding='UTF8') as f:
                    w = csv.writer(f)
                    w.writerow(Vl)
            else:
                open(f"Teams\\PitScouting{Vl[0]}.csv", "a").close()
                with open(f"Teams\\PitScouting{Vl[0]}.csv", "a", encoding='UTF8') as f:
                    w = csv.writer(f)
                    w.writerow(["Team Number"])
                w.writerow(Vl)
        else:
            print(f"Team {Vl[0]} match {Vl[1]} has been scanned")
